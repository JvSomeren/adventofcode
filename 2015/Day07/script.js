...
solve = ( wires, gate ) => {
  // console.log( gate, wires[gate]);

  if( typeof wires[gate] === "number")
    return wires[gate];

  const matches = wires[gate].match(/((\w{1,}) )?([A-Z]{1,}) (\w{1,})/);

  if(matches == null) {
    return solve(wires, wires[gate]);
  }

  switch(matches[3]) {
    case "AND":
      return uint16( solve(wires, matches[2]) ) & uint16( solve(wires, matches[4]) );
    case "OR":
      return uint16( solve(wires, matches[2]) ) | uint16( solve(wires, matches[4]) );
    case "LSHIFT":
      return uint16( solve(wires, matches[2]) ) << uint16( matches[4] );
    case "RSHIFT":
      return uint16( solve(wires, matches[2]) ) >> uint16( matches[4] );
    case "NOT":
      return ~ uint16( solve(wires, matches[4]) );
    default:
...
bitEmulator = (input) => {
  ...
  for(let instruction of input) {
    const target = instruction.match(/-> (\w{1,})/)[1];
    const operation = instruction.match(/(.{1,}) ->/)[1];

    wires[target] = isNaN(parseInt(operation)) ? operation : parseInt(operation);
  }
  console.log(wires);

  for(let gate in wires) {
    console.log( gate, uint16(solve(wires, gate)));
  }
  // console.log( uint16( solve(wires, 'bk') ) );
};
...
document.getElementById('answer-part-one').innerHTML = bitEmulator(input);
// document.getElementById('answer-part-two').innerHTML = ;
